...
for i = 1:numFiles
   ...
   %pkh = events(1);    % Peak knee height
    ...
    fs_new = fs*100/(mir-sfc);
    mer_master_i = round(mer_master*100);
    br_master_i = round(br_master*100);
    ...
    %% Calculate total trans/gen/abs over phases @ shoulder
        ...
        shldr_trans_jfp_cp(i,1) = trapz(shldr_trans_jfp(1:mer_master_i(i),i)) / fs_new;
        shldr_trans_jfp_ap(i,1) = trapz(shldr_trans_jfp(mer_master_i(i):br_master_i(i),i)) / fs_new;
        shldr_trans_jfp_dp(i,1) = trapz(shldr_trans_jfp(br_master_i(i):end,i)) / fs_new;
        
        shldr_trans_stp_cp(i,1) = trapz(shldr_trans_stp(1:mer_master_i(i),i)) / fs_new;
        shldr_trans_stp_ap(i,1) = trapz(shldr_trans_stp(mer_master_i(i):br_master_i(i),i)) / fs_new;
        shldr_trans_stp_dp(i,1) = trapz(shldr_trans_stp(br_master_i(i):end,i)) / fs_new;
        
        shldr_trans_net_cp(i,1) = trapz(shldr_trans_net(1:mer_master_i(i),i)) / fs_new;
        shldr_trans_net_ap(i,1) = trapz(shldr_trans_net(mer_master_i(i):br_master_i(i),i)) / fs_new;
        shldr_trans_net_dp(i,1) = trapz(shldr_trans_net(br_master_i(i):end,i)) / fs_new;
        
        shldr_gen_cp(i,1) = (trapz(shldr_gen_stp(1:mer_master_i(i),i)) + trapz(abs(shldr_gen_stp(1:mer_master_i(i),i)))) / (2*fs_new);
        shldr_gen_ap(i,1) = (trapz(shldr_gen_stp(mer_master_i(i):br_master_i(i),i)) + trapz(abs(shldr_gen_stp(mer_master_i(i):br_master_i(i),i)))) / (2*fs_new);
        shldr_gen_dp(i,1) = (trapz(shldr_gen_stp(br_master_i(i):end,i)) + trapz(abs(shldr_gen_stp(br_master_i(i):end,i)))) / (2*fs_new);
        
        shldr_abs_cp(i,1) = (trapz(shldr_gen_stp(1:mer_master_i(i),i)) - trapz(abs(shldr_gen_stp(1:mer_master_i(i),i)))) / (2*fs_new);
        shldr_abs_ap(i,1) = (trapz(shldr_gen_stp(mer_master_i(i):br_master_i(i),i)) - trapz(abs(shldr_gen_stp(mer_master_i(i):br_master_i(i),i)))) / (2*fs_new);
        shldr_abs_dp(i,1) = (trapz(shldr_gen_stp(br_master_i(i):end,i)) - trapz(abs(shldr_gen_stp(br_master_i(i):end,i)))) / (2*fs_new);
    ...
    %% Calculate total trans/gen/abs over phases @ elbow
        ...
        elb_trans_jfp_cp(i,1) = trapz(elb_trans_jfp(1:mer_master_i(i),i)) / fs_new;
        elb_trans_jfp_ap(i,1) = trapz(elb_trans_jfp(mer_master_i(i):br_master_i(i),i)) / fs_new;
        elb_trans_jfp_dp(i,1) = trapz(elb_trans_jfp(br_master_i(i):end,i)) / fs_new;
        
        elb_trans_stp_cp(i,1) = trapz(elb_trans_stp(1:mer_master_i(i),i)) / fs_new;
        elb_trans_stp_ap(i,1) = trapz(elb_trans_stp(mer_master_i(i):br_master_i(i),i)) / fs_new;
        elb_trans_stp_dp(i,1) = trapz(elb_trans_stp(br_master_i(i):end,i)) / fs_new;
        
        elb_trans_net_cp(i,1) = trapz(elb_trans_net(1:mer_master_i(i),i)) / fs_new;
        elb_trans_net_ap(i,1) = trapz(elb_trans_net(mer_master_i(i):br_master_i(i),i)) / fs_new;
        elb_trans_net_dp(i,1) = trapz(elb_trans_net(br_master_i(i):end,i)) / fs_new;
        
        elb_gen_cp(i,1) = (trapz(elb_gen_stp(1:mer_master_i(i),i)) + trapz(abs(elb_gen_stp(1:mer_master_i(i),i)))) / (2*fs_new);
        elb_gen_ap(i,1) = (trapz(elb_gen_stp(mer_master_i(i):br_master_i(i),i)) + trapz(abs(elb_gen_stp(mer_master_i(i):br_master_i(i),i)))) / (2*fs_new);
        elb_gen_dp(i,1) = (trapz(elb_gen_stp(br_master_i(i):end,i)) + trapz(abs(elb_gen_stp(br_master_i(i):end,i)))) / (2*fs_new);
        
        elb_abs_cp(i,1) = (trapz(elb_gen_stp(1:mer_master_i(i),i)) - trapz(abs(elb_gen_stp(1:mer_master_i(i),i)))) / (2*fs_new);
        elb_abs_ap(i,1) = (trapz(elb_gen_stp(mer_master_i(i):br_master_i(i),i)) - trapz(abs(elb_gen_stp(mer_master_i(i):br_master_i(i),i)))) / (2*fs_new);
        elb_abs_dp(i,1) = (trapz(elb_gen_stp(br_master_i(i):end,i)) - trapz(abs(elb_gen_stp(br_master_i(i):end,i)))) / (2*fs_new);
...
%% Plot net transfer @ shoulder & elbow for visual inspection
...
clear ans br data events fs fs_new i j k mer minSTP mir sfc Sign time time_old
clc;
